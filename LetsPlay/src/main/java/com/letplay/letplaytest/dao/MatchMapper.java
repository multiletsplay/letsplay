...
public interface MatchMapper {
	...
	@Select( {"<script>",
	    ...
	    "       LEFT OUTER JOIN MEMBER mb ON m.ID=mb.ID ",
	    " <where>",
	    "   <if test='searchRegion1 != null and searchRegion1 != \"\"'>m.MATCH_LOCATION LIKE CONCAT(#{searchRegion1},'%') </if> ",
	    "   <if test='searchRegion1 == null or searchRegion1 == \"\"'>1=1 </if>",
	    "   <if test='searchRegion2 != null and searchRegion2 != \"\"'>AND m.MATCH_LOCATION LIKE CONCAT('%',#{searchRegion2},'%') </if>",
	    "   <if test='searchDate != null'>OR m.MATCH_DAY=#{searchDate} </if>",
	    "   <if test='searchTotal != null'>AND m.MATCH_TOTAL=#{searchTotal} </if> ",  // AND 조건으로 수정
	    "   <if test='searchStartTime != null'>OR m.START_TIME=#{searchStartTime} </if> ",
	    "   <if test='searchFacStatus != null'>OR m.MATCH_FACILITY=#{searchFacStatus} </if> ",
	    " </where> ",
	    " GROUP BY m.MATCH_SEQ ",
